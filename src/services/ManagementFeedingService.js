...
const ManagementFeedingService = {
    create: async function(data, isAuthenticated) {
      ...
      update: async function(data, isAuthenticated) {
        ...
        if (isAuthenticated) config = await AuthenticationService.checkAndGetToken();
        return axios
          .put(end.URL + end.VERSION + end.MANAGEMENT + end.FEED, data, config)
          .then(handleResponse);
      },
      delete: async function(id, entityId, isAuthenticated) {
        ...
        return axios
          .delete(
            end.URL + end.VERSION + end.MANAGEMENT + end.FEED + `?id=${id}&entityId=${entityId}`,
            config,
          )
          .then(handleResponse);
      },
...
export default ManagementFeedingService;