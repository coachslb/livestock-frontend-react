...
const ManagementBirthRegistrationService = {
    create: async function(data, isAuthenticated) {
        ...
        return axios
          .post(end.URL + end.VERSION + end.MANAGEMENT + end.BIRTH_REGISTRATION, data, config)
          .then(handleResponse);
      },
      update: async function(data, isAuthenticated) {
        ...
        if (isAuthenticated) config = await AuthenticationService.checkAndGetToken();
        return axios
          .put(end.URL + end.VERSION + end.MANAGEMENT + end.BIRTH_REGISTRATION, data, config)
          .then(handleResponse);
      },
      delete: async function(id, entityId, isAuthenticated) {
        ...
        return axios
          .delete(
            end.URL + end.VERSION + end.MANAGEMENT + end.BIRTH_REGISTRATION + `?id=${id}&entityId=${entityId}`,
            config,
          )
          .then(handleResponse);
      },
